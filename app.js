...
async function testProxy(platform,button){const input=$(platform==='youtube'?'#youtube-proxy':'#bilibili-proxy'),out=$('#proxy-test-result'),label=platform==='youtube'?'YouTube':'B站';button.disabled=true;const original=button.textContent;button.textContent='检测中…';out.textContent=`${label}：正在检测…`;try{const r=await fetch('/api/proxy-test',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({platform,value:normalizeProxy(input.value)})}),d=await r.json();if(!r.ok)throw Error(d.error||'检测失败');out.textContent=`${label}：`+d.steps.map(s=>`${s.name}${s.ok===true?' 通过':s.ok===false?' 未通过':' 未验证'}——${s.detail}${s.ms==null?'':`（${s.ms}ms）`}`).join('；')}catch(e){out.textContent=`${label}检测失败：${e.message}`}finally{button.disabled=false;button.textContent=original}}
$('#test-youtube-proxy').addEventListener('click',event=>testProxy('youtube',event.currentTarget));
$('#test-bilibili-proxy').addEventListener('click',event=>testProxy('bilibili',event.currentTarget));
$('#save-update-frequency').addEventListener('click',async()=>{const days=Number($('#update-frequency').value);const r=await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({update_check_days:days})}),d=await r.json();if(!r.ok){showToast(d.error||'周期保存失败');return}showToast(days?`自动检查周期已设为每 ${days} 天`:'已关闭自动检查更新');refreshUpdateStatus(false)});loadUpdateFrequency();refreshUpdateStatus(false);loadProxies();refreshAbout();
async function refreshAbout(){const version=$('#about-version'),build=$('#about-build');try{const r=await fetch('/api/health'),d=await r.json();if(!r.ok)throw Error('版本信息读取失败');version.textContent=`v${d.version||'未知'}`;const b=d.build||{};build.textContent=b.tracked?`${b.sha}${b.time?` · ${b.time}`:''}`:'未知（本地构建或热部署）'}catch(e){build.textContent=e.message}}
...
async function loadRuntimeConfig(){try{const c=await (await fetch('/api/config')).json();$('#config-concurrency').value=String(c.concurrency||2);$('#config-metadata').checked=!!c.metadata;$('#config-thumbnail-jpg').checked=c.thumbnail_jpg!==false;$('#default-queue').checked=!!c.queue_mode;$('#metadata').checked=!!c.metadata}catch(e){}}
$('#save-storage-config').addEventListener('click',async()=>{const body={concurrency:Number($('#config-concurrency').value),metadata:$('#config-metadata').checked,thumbnail_jpg:$('#config-thumbnail-jpg').checked,queue_mode:$('#default-queue').checked};const r=await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),d=await r.json();if(!r.ok){showToast(d.error||'下载设置保存失败');return}$('#metadata').checked=!!d.metadata;showToast('下载设置已保存')});loadRuntimeConfig();
