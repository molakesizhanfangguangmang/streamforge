...
refreshPlugins();
$('#backup-now').addEventListener('click',async()=>{const include=['config','cookies','proxies','plugins'];try{const r=await fetch('/api/backup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({include})}),d=await r.json();if(!r.ok)throw Error(d.error||'备份失败');showToast(`已创建备份：${d.name}`)}catch(e){showToast(e.message)}});$('#restore-backup').addEventListener('click',async()=>{try{const list=await (await fetch('/api/backups')).json();const name=prompt(`输入要恢复的备份文件名：\n${list.map(x=>x.name).join('\n')}`);if(!name)return;if(!confirm('恢复会覆盖当前配置、Cookie、代理和插件。继续吗？'))return;const r=await fetch('/api/restore',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,include:['config','cookies','proxies','plugins']})}),d=await r.json();if(!r.ok)throw Error(d.error||'恢复失败');showToast('恢复完成，必要时请重启服务')}catch(e){showToast(e.message)}});async function refreshUpdateStatus(){const project=$('#project-update-status'),ytdlp=$('#ytdlp-update-status'),projectButton=$('#update-project'),ytButton=$('#update-ytdlp');try{const r=await fetch('/api/updates'),d=await r.json();if(!r.ok)throw Error(d.error||'更新状态读取失败');const release=d.release||{},yt=d.ytdlp||{},state=d.state||{};if(d.up_to_date){project.textContent=`当前 v${d.current_version}，已是最新版本`;projectButton.disabled=true;projectButton.textContent='已是最新'}else if(release.ok&&release.available){project.textContent=`当前 v${d.current_version||'未知'}，最新 ${release.tag} 可更新`;projectButton.disabled=false;projectButton.textContent='更新项目'}else{project.textContent=release.ok?`最新版本 ${release.tag} 缺少校验资产，暂不可更新`:'无法读取官方 Release';projectButton.disabled=true;projectButton.textContent='暂不可更新'}if(yt.ok&&yt.up_to_date){ytdlp.textContent=`当前 ${yt.current}，已是最新版本`;ytButton.disabled=true;ytButton.textContent='已是最新'}else if(yt.ok&&yt.available){ytdlp.textContent=`当前 ${yt.current||'镜像内置'}，最新 ${yt.tag}`;ytButton.disabled=false;ytButton.textContent='更新 yt-dlp'}else{ytdlp.textContent='yt-dlp 官方资产或校验文件不可用';ytButton.disabled=true;ytButton.textContent='暂不可更新'}if(state.running){project.textContent='更新正在执行…';projectButton.disabled=true;ytButton.disabled=true}}catch(e){project.textContent=e.message;ytdlp.textContent='更新服务不可用';projectButton.disabled=true;ytButton.disabled=true}}
...
$('#update-project').onclick=startHostUpdate;$('#update-ytdlp').onclick=startYtdlpUpdate;refreshUpdateStatus();setInterval(refreshUpdateStatus,5000);$('#clear-logs').addEventListener('click',()=>showToast('日志已清空'));
