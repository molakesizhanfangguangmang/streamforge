  ...
  async function poll(session){
    ...
    try{
      ...
      if(data.state==='success'){
        stopQr();state.textContent=`登录成功，当前账号：${data.account?.name||'已登录'}（UID ${data.account?.uid??'未知'}）`;
        await refreshAuth();return;
      }
