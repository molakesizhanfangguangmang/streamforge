...
$('#save-proxies').addEventListener('click',async()=>{const proxies={youtube:$('#youtube-proxy').value.trim(),bilibili:$('#bilibili-proxy').value.trim()},bad=Object.entries(proxies).find(([,v])=>v&&!/^(https?|socks5h?|socks4a?):\/\/[^\s/]+$/i.test(v));if(bad){showToast(`${bad[0]==='youtube'?'YouTube':'B站'}代理地址要写成 http://127.0.0.1:7890 这种带协议头的形式`);return}const r=await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({proxies})}),d=await r.json();if(!r.ok){showToast(d.error||'代理保存失败');return}renderProxyNote(d.proxies);showToast('代理设置已保存，之后的解析与下载立即生效')});
